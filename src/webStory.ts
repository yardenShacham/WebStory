...
export class WebStory {
    ...
    constructor(settings: Settings) {
        ...
        this.defaultTemplateName = settings.defaultTemplateName ? settings.defaultTemplateName : "purpleFlower";
    ...
    private  setPagesWithDefaultTemplate(pages: Page[]): Page[] {
        ...
        for (let i = 0; i < pages.length; i++) {
            if (!pages[i].pageContainer) {
                ...
                if (!pages[i].template)
                    pages[i].template = <any>require(`./webStoryDefaultTemplates/welcomePage.html`);
            }
        ...
        if (!isModalDefiend) {
            newPages.unshift({
                ...
                pageContainer: null,
                template: <any>require(`./webStoryDefaultTemplates/welcomePage.html`)
            });
        }
    ...
    private getDefaultTemplate(isLastPage: boolean): string {
        if (isLastPage)
            return <any>require(`./webStoryDefaultTemplates/${this.defaultTemplateName}-lastPage.html`);

        return <any>require(`./webStoryDefaultTemplates/${this.defaultTemplateName}.html`);
    }
}